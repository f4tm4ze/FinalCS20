...
from model_loader import load_all_models, load_model
...
<style>
    ...
    .intel-panel {
        background: var(--error-dim); border: 1px solid rgba(248,113,113,0.25);
    ...
    .intel-val.tag { display:inline-block; background:rgba(248,113,113,0.15); border:1px solid rgba(248,113,113,0.3); border-radius:4px; padding:1px 7px; font-size:0.6rem; margin:1px 2px; color:var(--error); }
    ...
    .source-badge {
        ...
        font-family:'Space Mono',monospace; font-size:0.6rem; color:var(--error); margin:2px;
    }
    ...
    .severity-badge { font-family: 'Space Mono', monospace; font-size: 0.58rem; font-weight: 700; letter-spacing: 0.1em; padding: 2px 8px; border-radius: 4px; }
    ...
    .rule-row { display: flex; align-items: center; gap: 10px; padding: 0.45rem 0; border-bottom: 1px solid var(--border); font-family: 'Space Mono', monospace; font-size: 0.65rem; }
    ...
    .severity-badge-miss { font-family: 'Space Mono', monospace; font-size: 0.58rem; font-weight: 700; letter-spacing: 0.1em; padding: 2px 8px; border-radius: 4px; background: rgba(71,85,105,0.15); color: var(--text-muted); border: 1px solid rgba(71,85,105,0.25); }
    .h-counts { display: flex; gap: 6px; margin-top: 0.85rem; }
    .h-count-chip { font-family: 'Space Mono', monospace; font-size: 0.6rem; padding: 3px 10px; border-radius: 20px; }
    ...
    .confidence-panel::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 1px; background: linear-gradient(90deg, transparent, var(--border-bright), transparent); }
    .conf-header { display: flex; align-items: baseline; justify-content: space-between; margin-bottom: 0.85rem; }
    .conf-title { font-family: 'Space Mono', monospace; font-size: 0.62rem; letter-spacing: 0.2em; text-transform: uppercase; color: var(--text-muted); }
    .conf-value { font-family: 'Space Mono', monospace; font-size: 1.5rem; font-weight: 700; line-height: 1; }
    .conf-track { position: relative; height: 8px; border-radius: 99px; overflow: visible; margin-bottom: 1.6rem; }
    .conf-fill  { position: absolute; top: 0; left: 0; height: 100%; border-radius: 99px; transition: width 0.6s cubic-bezier(0.4,0,0.2,1); }
    .conf-glow-head { position: absolute; top: 50%; transform: translateY(-50%); width: 4px; height: 16px; border-radius: 2px; }
    .conf-tick { position: absolute; top: 14px; transform: translateX(-50%); display: flex; flex-direction: column; align-items: center; }
    .conf-tick::before { content: ''; display: block; width: 1px; height: 5px; background: var(--text-muted); opacity: 0.4; margin-bottom: 3px; }
    .conf-tick-label { font-family: 'Space Mono', monospace; font-size: 0.5rem; color: var(--text-muted); opacity: 0.6; }
    .conf-sub { font-family: 'Space Mono', monospace; font-size: 0.6rem; color: var(--text-muted); letter-spacing: 0.05em; margin-top: 0.25rem; }
</style>
...
def render_intel_panel(intel: ThreatIntel):
    ...
    sources_html = "".join(f'<span class="source-badge">⚑ {s}</span>' for s in intel.sources)
    ...
    vt_row       = f'<div class="intel-row"><span class="intel-key">VT DETECT</span><span class="intel-val" style="color:var(--error);">{intel.vt_malicious} / {intel.vt_total} engines</span></div>' if intel.vt_total > 0 else ""
...
def render_results(prediction, confidence, filename, model_used, intel: ThreatIntel, sha256: str, heuristic: HeuristicResult):
    ...
    <div class="confidence-panel">
        ...
        <div class="conf-track" style="background:{track_color};">
            <div class="conf-fill" style="width:{conf_pct}%;background:linear-gradient(90deg,{bar_color}88,{bar_color});box-shadow:0 0 12px {glow_color};"></div>
            <div class="conf-glow-head" style="left:calc({conf_pct}% - 2px);background:{bar_color};box-shadow:0 0 8px {glow_color};"></div>
            {ticks_html}
        </div>
        <div class="conf-sub">
...
with st.sidebar:
    ...
    selected_model = st.selectbox("", model_names, index=0, label_visibility="collapsed")
    ...
    <div class="models-pill">
        ...
        <span class="pill-label">Models available</span>
    </div>
    """, unsafe_allow_html=True)
...
with center:
    ...
    if uploaded_file is not None:
        ...
        if st.button("🔍 Run Analysis", use_container_width=True):
            ...
            # Step 1: Hash lookup
            with st.spinner("Step 1/3 — Hashing file and querying threat databases..."):
                try:
            ...
            # Step 2: Load model (lazy, with download if needed)
            with st.spinner(f"Step 2/3 — Loading {selected_model} model..."):
                try:
                    ...
                    if model is None:
                        ...
                        if model is None:
                            raise ValueError(
                                f"Could not load '{selected_model}'. "
                                "Check that the .pkl file is in your GitHub repo under models/."
                            )
                        models[selected_model] = model
                except Exception as e:
                    error = e

            # Step 3: Feature extraction + prediction
            if not error:
                with st.spinner("Step 3/3 — Extracting features and running ML + heuristics..."):
                    try:
